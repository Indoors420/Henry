...
def nounGen(a): #Returns a noun/object for use in random phrase generation
    ...
    if chance > 11:
        adj = adjectiveGen(random.randint(1,3))
    else:
        adj = ""
    nouns1 = [
        f"this {adj}sandwich",
        f"the {adj}CarFax™",
        f"the {adj}cheese tub",
        f"Harvard",
        f"the {adj}president",
        f"an {adj}orphanage",
        f"some {adj}battery acid",
        f"some {adj}farm animals",
        f"some {adj}shit",
        f"these {adj}cheez-its™",
        f"the {adj}economy",
        f"the {adj}man-man",
        f"the {adj}weird kid",
        f"the {adj}IRS",
        ...
        f"these {adj}niggas",
        f"Alexa",
        f"the {adj}christian minority in Afghanistan",
        f"some {adj}catholic priests",
        f"minecraft",
        f"Russ",
        f"Wal-Mart customer service",
        f"Fortnite",
        f"my neighbor's {adj}teenage son",
        f"the rap game",
        f"communism",
        f"the {adj}fellas",
        f"Dwayne Johnson",
        f"Allah",
        f"a {adj}microwave",
        f"an adult polar bear",
        f"a {adj}pirated copy of Shrek 2",
        f"some {adj}alphabet soup",
    ]
    nouns2 = [ #"Interest" nouns
        f"{adj}juicing",
        f"{adj}cryptocurrency",
        f"{adj}sexual harassment",
        f"{adj}wakeboarding",
        f"{adj}golf",
        f"{adj}arson",
        f"{adj}dentistry",
